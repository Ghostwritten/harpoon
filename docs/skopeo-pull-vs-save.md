...
- **Cleanup:** `defer os.RemoveAll(tempDir)` — directory is removed when the function returns
...
| Operation   | Command                                      | Destination format   | Location      | Persistent | Inspectable |
|------------|-----------------------------------------------|----------------------|---------------|------------|-------------|
| `hpn pull` | `skopeo copy docker://image dir:/tmp/...`    | `dir:`               | Temp directory| No         | No          |
| `hpn save` | `skopeo copy docker://image docker-archive:…`| `docker-archive:`    | User path     | Yes        | Yes         |
